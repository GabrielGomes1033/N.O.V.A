...
from core.runtime_guard import checar_rate_limit
from core.runtime_guard import validar_token
...
def rate_limit(limit: int = 120, window: int = 60):
    ...
    Usage:
        @router.get("/path", dependencies=[Depends(rate_limit(90))])
        def my_endpoint(): ...
    ...
    async def _check(request: Request):
        ...
        chave = f"{ip}:{path}"
        ok, retry = checar_rate_limit(chave=chave, limite=limit, janela_s=window)
        if not ok:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail={"error": "rate_limited", "retry_after_s": retry},
            )
...
def require_token():
    ...
    async def _check(request: Request):
        if validar_token(request.headers):
            return True
        raise HTTPException(
...
def require_rbac(*allow: str, error_detail: str = "rbac_forbidden"):
    ...
    async def _check(request: Request):
        ...
        user = str(request.headers.get("X-User-Name", "") or "").strip()
